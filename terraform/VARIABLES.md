...
| `openshift_version` | 4.10.37 | Openshift Cluster version |
...
| `az` | single_zone | The number of Availability Zones to be used for the deployment. Keep in mind that some Regions may be limited to two Availability Zones. For an OCP cluster to be highly available, three Availability Zones are needed to avoid a single point of failure. Allowed values: `single_zone` and `multi_zone`. |
...
|OCP|
| `existing_cluster` | Requires input | Set true if you already have a running Openshift Cluster and you only want to install IBM Z & Cloud Modernization Stack |
