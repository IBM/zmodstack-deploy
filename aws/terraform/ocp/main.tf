...
oc get secret/pull-secret -n openshift-config -o jsonpath='{.data.\.dockerconfigjson}' --kubeconfig ${self.triggers.installer_workspace}/auth/kubeconfig | base64 -d | sed -e 's|:{|:{"${var.external_registry}":{"username":"${var.external_registry_username}","password":"${var.external_registry_password}"},|' > /tmp/dockerconfig.json
oc set data secret/pull-secret -n openshift-config --from-file=.dockerconfigjson=/tmp/dockerconfig.json --kubeconfig ${self.triggers.installer_workspace}/auth/kubeconfig
...
resource "null_resource" "configure_ibm_operator_catalog" {
  triggers = {
    installer_workspace     = local.installer_workspace
  }
  ...
  provisioner "local-exec" {
    command =<<EOF
oc apply -f ${self.triggers.installer_workspace}/ibm-catalogsource.yaml --kubeconfig ${self.triggers.installer_workspace}/auth/kubeconfig
EOF
}
  depends_on = [
    local_file.ibm_catalogsource,
    null_resource.install_openshift,
    null_resource.change_clb_to_nlb,
  ]
}
