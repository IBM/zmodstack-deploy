...
git clone --branch feature-ce-42 https://github.com/IBM/zmodstack-deploy.git $GIT_CLONE_DIR
...
chown -R $BOOTSTRAP_ADMIN_USERNAME:root $GIT_CLONE_DIR/ocp && runuser -l $BOOTSTRAP_ADMIN_USERNAME -c "cd $GIT_CLONE_DIR/ocp/ansible && \
